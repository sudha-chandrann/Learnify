...
function DeleteButton({ courseId }: DeleteButtonProps) {
  ...
  const handleDelete = async () => {
    try {
      setIsLoading(true);
      await axios.delete(`/api/generate/material/${courseId}/quiz`);
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription className="text-black/70">
            This action cannot be undone. This will permanently delete this quiz and all the data related to it. You will not be able to recover this data.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
