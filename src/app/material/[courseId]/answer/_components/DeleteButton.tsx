...
function DeleteButton({ courseId }: DeleteButtonProps) {
  ...
  const handleDelete = async () => {
    try {
      setIsLoading(true);
      await axios.delete(`/api/generate/material/${courseId}/qacollection`);
      toast.success("QAcollection is  deleted successfully");
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription className="text-black/70">
            This action cannot be undone. This will permanently delete these QAcollection. You will not be able to recover this data.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
