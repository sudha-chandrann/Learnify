...
export async function DELETE(
    ...
    try {
        ...
        if (!quiz) {
            return NextResponse.json({ message: "No quiz set found" }, { status: 404 });
        }

        // Check if the user is the owner of the flashcard set
        if (quiz.createdby !== userId) {
            return NextResponse.json({
                message: 'Forbidden: You do not have permission to delete this quiz material'
            }, { status: 403 });
        }
        ...
        return NextResponse.json(
            { message: "quiz set and all related data deleted successfully" },
            { status: 200 }
        );
    } catch (error) {
